...
class pGraphics::pButton : public pGraphics::pElement {
    private:
        int state = 0;
        std::vector<colors::pColor> colors;
        void(*function)(int);

    public:
        pButton(pGraphics* gfx, std::pair<float, float> pos, std::pair<float, float> size, std::vector<colors::pColor> colors, void(*function)(int));
        ~pButton();
        ...
        void setColors(std::vector<colors::pColor> newColors);
        ...
        std::vector<colors::pColor> getColors() { return colors; };
